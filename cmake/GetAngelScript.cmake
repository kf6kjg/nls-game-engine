...
set(NLS_ENGINE_LIBRARY "AngelScript")
...
if(DEFINED NLS_ENGINE_LIBRARY_MODE OR DEFINED NLS_ENGINE_BUILD_MODE)
	...
	# Get the extra addon
	load_library_source_from_web(
		"${NLS_ENGINE_LIBRARY}_addon_aswrapper"
		"${NLS_ENGINE_LIBRARY}/sdk/add_on" # no trailing slash
...
if(DEFINED NLS_ENGINE_LIBRARY_MODE)
	...
	execute_process(
		COMMAND cmake -E copy "${LIB_AngelScript_DIR}/sdk/angelscript/include/angelscript.h" "${LIBS_INCLUDE_PATH}/angelscript/"
		RESULT_VARIABLE COPY_RESULT
	)
	if(NOT "${COPY_RESULT}" MATCHES "^0")
		message("Copy of AS header failed!")
	...
	if("${AS_ADDONS}" STRGREATER "")
		...
		foreach(loop_addon ${AS_ADDONS})
			...
			if(NOT "${AS_HEADERS}" STRGREATER "")
				message("\tNo headers found...  Looking in '${LIB_AngelScript_DIR}/sdk/add_on/${loop_addon}/*.h'")
			endif(NOT "${AS_HEADERS}" STRGREATER "")
			foreach(loop_file ${AS_HEADERS})
				execute_process(
					COMMAND cmake -E copy ${loop_file} "${LIBS_INCLUDE_PATH}/angelscript/"
					RESULT_VARIABLE COPY_RESULT
				)
				if(NOT "${COPY_RESULT}" MATCHES "^0")
					message("\t\tCopy of add-on header '${loop_file}' failed!")
				endif(NOT "${COPY_RESULT}" MATCHES "^0")
...
if(DEFINED NLS_ENGINE_BUILD_MODE)
	...
	message("Setting up ${NLS_ENGINE_LIBRARY} for compilation as a static library.")
	...
	file(GLOB_RECURSE ${NLS_ENGINE_LIBRARY}_SOURCE
		"${LIB_AngelScript_DIR}/sdk/angelscript/source/as_*.cpp"
	)
	file(GLOB_RECURSE ${NLS_ENGINE_LIBRARY}_SOURCE_HEADERS
	...
	foreach(loop_addon ${AS_ADDONS})
		...
		set(${NLS_ENGINE_LIBRARY}_ADDON_SOURCE ${${NLS_ENGINE_LIBRARY}_ADDON_SOURCE} ${ADDON_SOURCE_GLOB})
		...
		set(${NLS_ENGINE_LIBRARY}_ADDON_SOURCE_HEADERS ${${NLS_ENGINE_LIBRARY}_ADDON_SOURCE_HEADERS} ${ADDON_HEADER_GLOB})
	endforeach(loop_addon)
	source_group("Source" FILES ${${NLS_ENGINE_LIBRARY}_SOURCE})
	source_group("Source Headers" FILES ${${NLS_ENGINE_LIBRARY}_SOURCE_HEADERS})
	source_group("Addon Source" FILES ${${NLS_ENGINE_LIBRARY}_ADDON_SOURCE})
	source_group("Addon Headers" FILES ${${NLS_ENGINE_LIBRARY}_ADDON_SOURCE_HEADERS})
	...
	add_library(${NLS_ENGINE_LIBRARY} STATIC ${${NLS_ENGINE_LIBRARY}_SOURCE} ${${NLS_ENGINE_LIBRARY}_SOURCE_HEADERS} ${${NLS_ENGINE_LIBRARY}_ADDON_SOURCE} ${${NLS_ENGINE_LIBRARY}_ADDON_SOURCE_HEADERS})
	
	if(MSVC)
		set_target_properties(${NLS_ENGINE_LIBRARY} PROPERTIES COMPILE_FLAGS "/MP")
	endif(MSVC)
